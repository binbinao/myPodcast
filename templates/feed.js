...
(function () {
  ...
  function renderLatest(groups) {
    ...
    // 一样的策略——避免被外层 #latest-list grid 当 child 错位塞到空 cell）。
  ...
  function renderSeries(groups) {
    ...
    for (var lp = 1; lp <= totalListPages; lp++) {
      ...
      html += '<div class="series-list-page" data-list-page="' + lp + '"'
        + (lp === 1 ? '' : ' hidden') + '>';
    ...
    // 注意：listPagerHtml 由 init() 单独调用 append 到 #series-list 的 parent
    // （不能写在 seriesSlot.innerHTML 里，否则被 series-list grid 当 child 错位）
    ...
    // 暴露给 init / click handler 用
    renderSeries._totalListPages = totalListPages;
    return html;
  }
  ...
  function bindSeriesPagination() {
    ...
    document.addEventListener('click', function (ev) {
      ...
      if (action === 'list-prev' || action === 'list-next' || action === 'list-num') {
        ...
        if (slot) {
          slot.querySelectorAll('.series-list-page').forEach(function (g) {
        ...
        pager.parentNode.replaceChild(newPager.firstChild, pager);
        return;
      }
    ...
    fetch('manifest.json', { cache: 'no-cache' })
      ...
      .then(function (data) {
        ...
        if (seriesSlot) {
          ...
          // 列表级 pager 必须挪出 #series-list grid 容器（series-list 本身是 grid，
          // pager 当 child 会被 grid auto-flow 错位塞到空 cell）。挂到 section 末尾，
          // 水平居中显示。
          if (renderSeries._totalListPages > 1 && seriesSlot.parentNode) {
            var pagerDiv = document.createElement('div');
            pagerDiv.innerHTML = listPagerHtml(renderSeries._totalListPages, 1);
            seriesSlot.parentNode.appendChild(pagerDiv.firstChild);
          }
        }
